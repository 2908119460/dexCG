...
def generate_plane_features(p, c, resolution, plane="xz"):
    ...
    xy = normalize_coordinate(
        p.clone(), plane=plane, padding=padding
    ...
    fea_plane = fea_plane.reshape(
        p.size(0), c_dim, resolution, resolution
...
def sample_triplane_feat(feature_triplane, normalized_pos):
    ...
    x_feat = F.grid_sample(
        tri_plane[0],
        torch.cat([normalized_pos[:, :, 0:1], normalized_pos[:, :, 1:2]], dim=-1).unsqueeze(dim=1),
        padding_mode="border",
        align_corners=True,
    )
    y_feat = F.grid_sample(
        tri_plane[1],
        torch.cat([normalized_pos[:, :, 1:2], normalized_pos[:, :, 2:3]], dim=-1).unsqueeze(dim=1),
        padding_mode="border",
        align_corners=True,
    )
    ...
    z_feat = F.grid_sample(
        tri_plane[2],
        torch.cat([normalized_pos[:, :, 0:1], normalized_pos[:, :, 2:3]], dim=-1).unsqueeze(dim=1),
        padding_mode="border",
        align_corners=True,
    )
...
class TriPlanePC2Encoder(torch.nn.Module):
    ...
    def __init__(
        ...
        if cfg.point_encoder_type == "pvcnn":
            self.pc_encoder = PVCNNEncoder(
                point_encoder_out_dim,
                device=self.device,
                in_channels=in_channels,
                use_2d_feat=use_2d_feat,
            )  # Encode it to a volume vector.
        elif cfg.point_encoder_type == "pointnet":
            # TODO the pointnet was buggy, investigate
            self.pc_encoder = ConvPointnet(
                c_dim=point_encoder_out_dim,
                dim=in_channels,
                hidden_dim=32,
    ...
    def encode(
        ...
        if self.cfg.point_encoder_type == "pvcnn":
            if mv_feat is not None:
                ...
                pc_feat, points_feat = self.pc_encoder(
                    point_cloud
                )  # 3D feature volume: BxDx32x32x32
            if self.cfg.use_point_scatter:
                ...
                pc_feat_1 = generate_plane_features(
                    point_cloud_xyz, points_feat_, resolution=self.z_triplane_resolution, plane="xy"
                )
                pc_feat_2 = generate_plane_features(
                    point_cloud_xyz, points_feat_, resolution=self.z_triplane_resolution, plane="yz"
                )
                pc_feat_3 = generate_plane_features(
